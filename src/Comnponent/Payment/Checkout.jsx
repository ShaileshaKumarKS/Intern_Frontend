...
const CheckoutForm = ({ selectedPlan }) => {
  ...
  const handleSubmit = async (event) => {
    ...
    if (error) {
      ...
      // Send payment method and selected plan to the server
      const response = await fetch('https://internareabackend-nrg6.onrender.com/api/profile/submit', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ paymentMethodId: paymentMethod.id, plan: selectedPlan }),
      });
